  			...
  			void Run(Agent* agent) override {
  				if (auto* cell = dynamic_cast<wd_cells*>(agent)) {
					...
					if (xyz[2] < min_b) {
    					...
    					/*
    					if (auto* mac_cell = dynamic_cast<macrophage*>(agent)) {
							real_t mp = 2.5 - pow((mac_cell->GetPosition()[1]-29.0),0.2)  ;
							mac_cell->SetProbability(mp);
							std::ofstream file_mp("mp.txt", std::ios::app);
							file_mp << std::fixed << std::setprecision(6) << mp << "\n";

							file_mp.close();	
    					}
    					if (auto* fbr_cell = dynamic_cast<fibroblast*>(agent)) {
							real_t fp = 2.5 - pow((fbr_cell->GetPosition()[1]-29.0),0.2)  ;
							fbr_cell->SetProbability(fp);
							std::ofstream file_fp("fp.txt", std::ios::app);
							file_fp << std::fixed << std::setprecision(6) << fp << "\n";
